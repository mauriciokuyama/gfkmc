...
def main(args):
    ...
    with open(features_file, 'a+') as f_file:
        ...
        for s in s_range:
            ...
            for k in k_range:
                anon_data = None
                anon_csv = f"{s_folder}/{dataset}_anon_k{k}.csv"
                if anon_method == AnonMethod.MONDRIAN:
                    print(anon_csv)
                    if os.path.exists(anon_csv):
                        anon_data = pd.read_csv(anon_csv)
                    else:
                        anon_data = get_result_one(ATT_TREES, raw_data, k, path, QI_INDEX, SA_INDEX, csv_file=csv_file)
                elif anon_method == AnonMethod.TDG:
                    if os.path.exists(anon_csv):
                        anon_data = pd.read_csv(anon_csv)
                    else:
                        anon_data = tdg_get_result_one(ATT_TREES, raw_data, k, path, QI_INDEX, SA_INDEX, csv_file=csv_file)
                elif anon_method == AnonMethod.CB:
                    if os.path.exists(anon_csv):
                        anon_data = pd.read_csv(anon_csv)
                    else:
                        anon_data = cb_get_result_one(ATT_TREES, raw_data, k, path, QI_INDEX, SA_INDEX, args.cb_alg, csv_file=csv_file)
